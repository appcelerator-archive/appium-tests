...
class Helper {
	...
	*/
	...
	/*
		...
		@param {Function} done -
	*/
	static buildTestApps(suites, tiSdk, done) {
		...
		suites.forEach(targetSuite => {
			p = p.then(() => {
				...
				if (!tiappMod[tiProj]) {
					...
					const tiappXmlFile = _makeAbs(path.join(targetSuite.name, tiProj, 'tiapp.xml'));
					change(tiappXmlFile);
				}
				...
				// -  --no-banner, --no-services, --no-colors
			});
		});
		...
		function appc(flags, next) {
			...
			// assume that the machine has appc cli
			console.log(`Running: 'appc ${flags.join(' ')}' ...`);
			const appcCmd = spawn('appc', flags);

			appcCmd.stdout.on('data', output => {
				console.log(output.toString());
			});

			appcCmd.stderr.on('data', output => {
				console.log(output.toString());
			});
