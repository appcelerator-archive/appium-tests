...
class Helper {
	...
	static launchGeny(genyDevice, done, stop) {
		...
		.then(() => {
			return new Promise(resolve => {
				...
				// NOTE: temporary
				playerCmd.stdout.on('data', output => {
					const
						line = output.toString().trim(),
						isLaunched = /^OpenGL connected to/g.test(line);
				...
				// but, need to read (stream) from genymotion-player.log.
				// the spawned player is printing stdout to stderr and the correct
				// log information doesn't appear in stdout until you manually kill genymotion emulator.
				const
					playerLog = path.join(os.homedir(), '.Genymobile', 'Genymotion', 'deployed', genyDevice, 'genymotion-player.log'),
					logStream = fs.createReadStream(playerLog),
					stalker = fs.watch(playerLog);
				...
				stalker.on('change', () => {
					...
					timeoutObj = setTimeout(() => {
						logStream.resume();
					}, 1000); // TODO: need a polling mechanism here
				});

				logStream.on('data', output => {
					logStream.pause();
					console.log(output.toString());
				});
	...
	*/
	static quitGeny(done) {
		/*
			NOTE:
				- this will only work on macOS because of osascript; will need to find a way on windows
				...
				- by running this apple script, it will make genymotion emulator gracefully die.

			use ChildProcess to gracefully "kill" genymotion emulator: osascript -e 'quit app "player"'
		*/
	}
